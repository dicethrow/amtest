...
class fpga_interface(lxdev.RemoteClient):
	...
	class upload_manager:
		def __init__(self, fpga_interface, local_filename):
			...
			self.abs_remote_filename = self.fpga_interface.get_remote_filename_from_local(self.local_filename)
		...
		def build_bitstream(self):
			cprint(f"build bitstream of {self.abs_remote_filename}...", "yellow")
			result, error = self.fpga_interface.execute_commands(
				[
					# f"cd {self.fpga_interface.get_remote_filename_from_local(os.path.dirname(self.abs_remote_filename))}",
					"PATH=$PATH:~/Documents/oss-cad-suite/bin", # for yosys, and any others?
					f"python3 {self.abs_remote_filename}", # defaults to upload if no arg passed
				],
		...
		def upload_bitstream(self):
			bitstream_filename = f'{self.abs_remote_filename.replace(".py", "_build")}/top.bit'
		...
		def remove_bitstream_files(self):
			local_bitstream_dir = self.abs_remote_filename.replace(".py", "_build")
	...
	class simulate_manager:
		def __init__(self, fpga_interface, local_filename):
			...
			self.abs_remote_filename = self.fpga_interface.get_remote_filename_from_local(self.local_filename, get_as_relative=True)
			
		def simulate_file(self):			
			cprint(f"simulate of {self.abs_remote_filename}...", "yellow")
...
class mcu_interface(lxdev.RemoteClient):
	...
	def do_task(self, task, local_filename=None):
		if task == "reflash-binary":
			...
			self.rsync_micropython_files_between(from_dir = amtest.boards.ulx3s.gui_ui.mcu_firmware.source.__file__, to_dir = "/pyboard/")
			# self.rsync_micropython_files_between(from_dir = "amtest/boards/ulx3s/gui_ui/common/test_common/", to_dir = "/pyboard/test_common/")
			self.rsync_micropython_files_between(from_dir = amtest.boards.ulx3s.gui_ui.common.test_common.__file__, to_dir = "/pyboard/test_common/")
